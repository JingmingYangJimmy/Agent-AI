...
const DOMAIN = "http://localhost:5001";//we can put inside .env
...
const ChatComponent = (props) => {
  ...
  useEffect(() => {
    if (!listening && Boolean(transcript)) {
      (async () => await onSearch(transcript))();//IIFE, arrow function
  ...
  const talk = (what2say) => {
    speech
      ...
      .then(() => {
        ...
        console.log("Success !");
        userStartConvo();//after AI said, AI start listening
      })
      .catch((e) => {
  ...
  const chatModeClickHandler = () => {
    ...
    setIsRecording(false);//always turn recording into false
    SpeechRecognition.stopListening();//then also stop listening
  ...
  return (
    <div style={searchContainer}>
      ...
      {isChatModeOn && (//in case of click accidentally, when chat mode 
      //off, we can choose to decide record or not
        <Button
          type="primary"
          icon={<AudioOutlined />}
          size="large"
...
//first show question, then show answer, how to do